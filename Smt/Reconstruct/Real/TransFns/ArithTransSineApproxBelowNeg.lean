...
theorem arithTransSineApproxBelowNegComp (d k : Nat) (x evalTaylor l u : Real) (hd : d =2*k + 1) (hx : x ≤ 0)
    (hev : evalTaylor = sinTaylor d x - x ^ (d + 1) / (d + 1).factorial) :
    (x ≥ l ∧ x ≤ u) → evalTaylor ≤ sin x := by
  intro _
  rw [<- sinEmbedding] at hev
  rw [hev]
  exact arithTransSineApproxBelowNeg d k x hd hx
